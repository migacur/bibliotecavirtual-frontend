...
const Agregar = () => {
  ...
  if (!user)
    return (
      <Aviso msg="Error 401 - No tienes permiso para acceder a esta ruta" />
    );
  ...
  const agregarLibro = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        Swal.fire("", "El libro fue agregado correctamente", "success");
      }
    ...
    <div className="form-container">
      ...
      >
        ...
        <input
          ...
          accept=".jpg, .jpeg, .png"
        />
        <input type="submit" value="Agregar libro" className="btn-send"></input>
      </form>
    </div>
  );
};
